...
\begin{frame}[fragile]{Trace Analysis}
  \inputminted[fontsize=\footnotesize]{java}{iterator.java}

  Motivating example: calls to \texttt{next()} must always follow a
  call to \texttt{hasNext()}.
  ...
  \begin{center}
    \begin{tikzpicture}[->, auto, node distance=8cm]
      ...
      \path (A) edge [bend left]  node {hasNext()} (B)
            (B) edge [bend left]  node {next()} (A);
    \end{tikzpicture}
  \end{center}
\end{frame}
