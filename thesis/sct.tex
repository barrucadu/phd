...
incomplete~\sref{sct-bounding}.  Finally we discuss two tools for
...
Maximal Causality Reduction (MCR)\cite{huang2015,huang2017} is an alternative to
...
scheduling decisions are made randomly, using a provided seed, and a
complete execution trace is returned, which can be rendered into a
graphical form showing the interactions between threads to aid
debugging.  The authors report that the graphical traces can often
suggest potential race conditions which have not been evident to a
human reader.
