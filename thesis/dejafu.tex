...
Given the current state, which we call the context, the identifier of
the chosen thread, and its primitive action, we either indicate a
failure condition, or produce a new context; in both cases we return a
log of what happened, to put into the execution trace returned to the
user.  The STM primitive actions also have an operational semantics.
As transactions are atomic, we only care about the big-step behaviour,
which simplifies some aspects of their implementation.
...
To simplify the presentation of our semantic rules, in this section we
use a pure model of the heap as a heterogeneous map.

\paragraph{Syntax}
We express our concurrency semantics as transitions on a context
$\langle\texttt{C}, \texttt{B}, \texttt{H}, \texttt{T}\rangle$:

\begin{itemize}
\item \verb|C| is the number of capabilities, a natural number.
\item \verb|B| is the relaxed-memory buffer, the representation
  depends on the memory model.
\item \verb|H| is the heap, a heterogeneous map from identifiers to
  Haskell values.
\item \verb|T| is a map from thread identifiers to threads
  $\langle\texttt{K}, \texttt{E}, \texttt{M}\rangle$:
  \begin{itemize}
  \item \verb|K| is the next primitive action to execute.
  \item \verb|E| is the stack of exception handler functions.
  \item \verb|M| is the exception masking state.
  \end{itemize}
\end{itemize}
...
data Action n
  ...
  = AFork ((forall b. ConcT n b -> ConcT n b) -> Action n) (ThreadId -> Action n)
  ...
  | forall a.   ANewIORef                      a (ModelIORef n a -> Action n)
  ...
  -- MVars
  | forall a. ANewMVar                       (ModelMVar n a -> Action n)
...
parameter to the \verb|Action| type, which we use to provide the
mutable variables we use for model \verb|IORef|s, \verb|MVar|s, and
\verb|TVar|s.  This monad is usually \verb|IO|, but can be any
\verb|MonadConc|.

\dejafu{} communicates the result of the computation back to the
caller by inserting an \verb|ALift| action which writes the result to
a mutable variable at the end of the main thread.  Our implementation
of model execution is more similar to cooperative multitasking than
pre-emptive multitasking: if evaluating a primitive action fails to
terminate, the entire computation locks up.
...
execution trace and we omit the definitions of helper functions, but
explain them when first used.
...
$\langle\texttt{C}, \texttt{B}, \texttt{H}, \texttt{T}_{\left[\texttt{tid} \mapsto \semfullthread{AFork act c}{E}{m0}\right]}\rangle\rightarrow$&
$\langle\texttt{C}, \texttt{B}, \texttt{H}, \texttt{T}_{[\texttt{tid} \mapsto \semfullthread{c id}{E}{m0};~\texttt{id} \mapsto \texttt{new}]}\rangle$\\
...
leave the relaxed-memory buffer and heap untouched.  We use
``\verb|id| is fresh'' to mean that \verb|id| is a new unique
identifier, which we use when creating threads, \verb|IORef|s, and
\verb|MVar|s.  We write $\texttt{M}_{[\texttt{k} \mapsto \texttt{v}]}$
to denote a new map which maps the key \texttt{k} to the value
\texttt{v}.
...
\verb|forkWithUnmask|, and implement the others in terms of this.  We
do not model which capability a thread is executing on, so we do not
also need an ``\verb|AForkOn|'' action.
...
In \verb|AStop|, we use
$\texttt{T}_{[\texttt{tid}\mapsto\varnothing]}$ to mean that the given
thread is deleted.
...
buffered writes nondeterministically committed, in some order
...
We present the rules for writing and committing to an \verb|IORef|,
which depends on the memory model, in
\Cref{sec:dejafu-semantics-writes}
...
some of them may cause the thread to block.  A thread can enforce a
memory barrier even if it blocks.  We model blocking by not changing
the continuation for the chosen thread.
...
\centering
\begin{tabular}{r@{\hspace{0.5em}}l}
\multicolumn{2}{r}{$\semlrule{ANewMVar c} \rightarrow \langle\texttt{C}, \texttt{B}, \texttt{H}_{[\texttt{id} \mapsto \texttt{Nothing}]}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c id}]}\rangle$}\\
\multicolumn{2}{l}{where \texttt{id} is fresh}\\
& \\
...
The behaviour of this operator depends on the memory model:
...
\item Under partial store order, there is a buffer for each (thread,
  \verb|IORef|) pair, but the process of performing the writes is
  otherwise the same.
\end{itemize}
