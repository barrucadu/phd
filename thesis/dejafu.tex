...
instance Monad n => MonadConc (ConcT r n) where
  ...
  -- other operations omitted
  \end{minted}
  \caption{The implementation of \lstref{monadconc} we use for testing.}\label{lst:mvarops}
\end{listing}
...
\lstref{100perfect} shows an implementation of these behaviours.
...
the defaults will suffice for most users.  \lstref{autocheck} shows
...
(\lstref{dejafuDiscard}), which exposes all the controls.
...
autocheck :: (Eq a, Show a) => (forall t. ConcST t a) -> IO Bool
  \end{minted}
  \caption{The type of the \texttt{autocheck} function.}\label{lst:autocheck}
\end{listing}

\begin{listing}[t]
  \centering
  \begin{minted}{haskell}
dejafuDiscard :: Show a => (Either Failure a -> Maybe Discard) -> Way -> MemType
              -> (forall t. ConcST t a) -> (String, Predicate a) -> IO Bool
  \end{minted}
  \caption{The type of the \texttt{dejafuDiscard} function.}\label{lst:dejafuDiscard}
\end{listing}
