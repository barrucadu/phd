...
log of what happened, to produce an execution trace for the user.  Our
...
identifier.  We write $\texttt{M}_{[\texttt{k} \mapsto \texttt{v}]}$
...
others.  \verb|ALift| causes a Haskell-level effect to occur,
producing a continuation by executing some \verb|IO| action.
\verb|AStop| has no continuation, we use
...
some of them may cause the thread to block.  An action can enforce a
