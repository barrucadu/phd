...
\begin{itemize}
\item \verb|C|, the number of capabilities.
\item \verb|B|, the relaxed-memory buffer state.
\item \verb|H|, the heap, a heterogeneous map from identifiers to
  values.
\item \verb|T|, a mapping from thread identifiers to threads.
\end{itemize}

We express threads as a record type containing:

\begin{itemize}
\item \verb|K|, the current continuation.
\item \verb|I|, the set of identifiers the thread is blocked on.
\item \verb|E|, the exception handler stack.
\item \verb|M|, the exception masking state.
\end{itemize}

We write $\texttt{H}_{[\texttt{id} \mapsto \texttt{v}]}$ to denote a
new heap which maps \texttt{id} to the value \texttt{v}, and
$\texttt{T}_{[\texttt{id}.\texttt{K} \mapsto \texttt{c}]}$ to denote a
new thread map where the continuation of thread \texttt{id} is now
\texttt{c}, and similarly for other fields.  There are two global
values which are in scope for every rule: (1)~\verb|memtype|, the
memory model for relaxed memory operations; and (2)~\verb|tid|, the
...
\Cref{fig:sem_cref} shows the semantics for the \verb|CRef|
...
interrupted with an exception.  We use
$\texttt{T}_{[\ldots]~[\ldots]~\ldots}$ as a shorthand for applying
multiple updates to the thread map.
...
& \\
$\langle\texttt{C}, \texttt{B}, \texttt{H}, \texttt{T}\rangle
\xrightarrow{\texttt{\small AReadCRefCAS id c}}$&
...
& \\
$\langle\texttt{C}, \texttt{B}, \texttt{H}, \texttt{T}\rangle
\xrightarrow{\texttt{\small AWriteCRef id a c}}$&
...
& \\
$\langle\texttt{C}, \texttt{B}, \texttt{H}, \texttt{T}\rangle
\xrightarrow{\texttt{\small AModCRef id f c}}$&
...
& \\
$\langle\texttt{C}, \texttt{B}, \texttt{H}, \texttt{T}\rangle
\xrightarrow{\texttt{\small AModCRefCAS id f c}}$&
$\langle\texttt{C}, \varnothing, (\texttt{H} \oplus \texttt{B})_{[\texttt{id} \mapsto \texttt{(n+1, f a)}]}, \texttt{T}_{[\texttt{tid}.\texttt{K} \mapsto \texttt{c ()}]}\rangle$ \\
...
\xrightarrow{\texttt{\small ACasCRef id (Ticket id n \_) a c}}$}\\
\multicolumn{2}{l}{if $\texttt{n} = \texttt{n0}$ \hfill $\langle\texttt{C}, \varnothing, (\texttt{H} \oplus \texttt{B})_{[\texttt{id} \mapsto \texttt{(n+1, a)}]}, \texttt{T}_{[\texttt{tid}.\texttt{K} \mapsto \texttt{c (True, Ticket id (n+1) a)}]}\rangle$}\\
\multicolumn{2}{l}{where $(\texttt{H} \oplus \texttt{B})_{[\texttt{id}]} = \texttt{(n0, \_)}$}\\
& \\
\multicolumn{2}{l}{$\langle\texttt{C}, \texttt{B}, \texttt{H}, \texttt{T}\rangle
\xrightarrow{\texttt{\small ACasCRef id (Ticket id n \_) \_ c}}$}\\
\multicolumn{2}{l}{if $\texttt{n} \neq \texttt{n0}$ \hfill $\langle\texttt{C}, \varnothing, \texttt{H} \oplus \texttt{B}, \texttt{T}_{[\texttt{tid}.\texttt{K} \mapsto \texttt{c (False, Ticket id n0 a0)}]}\rangle$}\\
...
\multicolumn{2}{r}{if $\texttt{H}_{[\texttt{id}]} = \texttt{Just x}$ \hfill $\langle\texttt{C}, \varnothing, \texttt{H}_{[\texttt{id} \mapsto \texttt{Nothing}]} \oplus \texttt{B}, \texttt{T}_{[\texttt{tid}.\texttt{K} \mapsto \texttt{c (Just x)}]} \Uparrow \{\texttt{id}\}\rangle$}\\
& \\
$\langle\texttt{C}, \texttt{B}, \texttt{H}, \texttt{T}\rangle
\xrightarrow{\texttt{\small ATryTakeMVar id c}}$&
