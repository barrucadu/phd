...
\item The \verb|getNumCapabilities| operation allows the programmer to
  query the number of capabilities.  If not otherwise set, we return
  two, despite executing everything in the same OS thread.  This is to
...
\verb|writeTVar|, so this would require a new primitive action.
...
library\cite{monad_par,marlow2011}, and the async library\cite{async}.
...
implement futures\cite{marlow2011}.  Despite its limitations, the
library can be effective in speeding up pure code\cite{marlow2011}.
