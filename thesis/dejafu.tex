...
\Cref{lst:gives} shows the provided \verb|gives| predicate, which is
...
more general than, the function \verb|always :: STM Bool -> STM ()|,
which registers an invariant to be checked at the end of STM
transactions.
...
we also want to know that.  We achieve this by reporting failure: the
result of a single execution of the program is a value of type
\verb|(Either Failure result, Trace)|.
...
\item \verb|H| is the heap, a heterogeneous map from identifiers to
  Haskell values.
\item \verb|T| is a mapping from thread identifiers to thread tuples.
  For a thread:
  \begin{itemize}
  \item \verb|K| is its current continuation.
  \item \verb|E| is its exception handler stack.
  \item \verb|M| is its exception masking state.
  \end{itemize}
\end{itemize}

We write $\texttt{M}_{[\texttt{k} \mapsto \texttt{v}]}$ to denote a
new map which maps the key \texttt{k} to the value \texttt{v}.  We use
\verb|memtype| to denote the memory model for relaxed memory
operations, which is \verb|SequentialConsistency|,
\verb|TotalStoreOrder|, or \verb|PartialStoreOrder|.  For some rules
we include side-conditions or local definitions.  We use Haskell
syntax for these.
...
$\semlrule{AFork act c}\rightarrow$&
$\langle\texttt{C}, \texttt{B}, \texttt{H}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c id};~\texttt{id} \mapsto \texttt{new}]}\rangle$\\
...
\multicolumn{2}{l}{\hphantom{where }$\texttt{new} = \texttt{Thread \{ K = act umask, E = [], M = m0 \}}$}\\
& \\
$\semlrule{ALift n}\rightarrow$&
...
some of them may cause the thread to block.  We omit semantic rules
for actions which would immediately block.
...
\centering
\begin{tabular}{r@{\hspace{0.5em}}l}
$\semlrule{ANewIORef a c}\rightarrow$&
...
& \\
$\semlrule{AWriteIORef id a c}\rightarrow$&
$\langle\texttt{C}, \texttt{B}, \texttt{H}_{[\texttt{id} \mapsto \texttt{(0, a)}]}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c ()}]}\rangle$ \\
\multicolumn{2}{l}{if $\texttt{memtype} = \texttt{SequentialConsistency}$}\\
& \\
$\semlrule{AWriteIORef id a c}\rightarrow$&
$\langle\texttt{C}, \texttt{B} + \texttt{(tid, id, a)}, \texttt{H}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c ()}]}\rangle$ \\
\multicolumn{2}{l}{if $\texttt{memtype} \neq \texttt{SequentialConsistency}$}\\
& \\
$\semlrule{AModIORef id f c}\rightarrow$&
$\langle\texttt{C}, \varnothing, (\texttt{H} \oplus \texttt{B})_{[\texttt{id} \mapsto \texttt{(n+1, f a)}]}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c ()}]}\rangle$ \\
...
\multicolumn{2}{l}{\hfill $\langle\texttt{C}, \varnothing, (\texttt{H} \oplus \texttt{B})_{[\texttt{id} \mapsto \texttt{(n+1, a)}]}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c (True, Ticket id (n+1) a)}]}\rangle$}\\
...
\multicolumn{2}{l}{\hfill $\langle\texttt{C}, \varnothing, \texttt{H} \oplus \texttt{B}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c (False, Ticket id n0 a0)}]}\rangle$}\\
\multicolumn{2}{l}{if $\texttt{n} \neq \texttt{n0}$}\\
\multicolumn{2}{l}{where $(\texttt{H} \oplus \texttt{B})_{[\texttt{id}]} = \texttt{(n0, a0)}$}\\
& \\
$\semlrule{ACommit idT idC}\rightarrow$&
$\langle\texttt{C}, \texttt{B'}, \texttt{H}_{[\texttt{idC} \mapsto \texttt{(n+1, a)}]}, \texttt{T}_{[\texttt{tid} \mapsto \varnothing]}\rangle$\\
\multicolumn{2}{l}{where $\texttt{H}_{[\texttt{idC}]} = \texttt{(n, \_)}$}\\
\multicolumn{2}{l}{\hphantom{where }$\texttt{B} - \texttt{(idT, idC)} = \texttt{(B', a')}$}
\end{tabular}
...
\centering
\begin{tabular}{r@{\hspace{0.5em}}l}
$\semlrule{ANewMVar c}\rightarrow$&
$\langle\texttt{C}, \texttt{B}, \texttt{H}_{[\texttt{id} \mapsto \texttt{Nothing}]}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c id}]}\rangle$ \\
\multicolumn{2}{l}{where \texttt{id} is fresh}\\
& \\
$\semlheaprule{APutMVar id a c}{Nothing}\rightarrow$&
$\langle\texttt{C}, \varnothing, \texttt{H}_{[\texttt{id} \mapsto \texttt{Just a}]} \oplus \texttt{B}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c ()}]}\rangle$\\
& \\
$\semlheaprule{ATakeMVar id c}{Just x}\rightarrow$&
$\langle\texttt{C}, \varnothing, \texttt{H}_{[\texttt{id} \mapsto \texttt{Nothing}]} \oplus \texttt{B}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c x}]}\rangle$\\
& \\
...
& \\
$\semlheaprule{ATryPutMVar id c}{Just \_}\rightarrow$&
$\langle\texttt{C}, \varnothing, \texttt{H} \oplus \texttt{B}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c False}]}\rangle$\\
& \\
$\semlheaprule{ATryPutMVar id a c}{Nothing}\rightarrow$&
$\langle\texttt{C}, \varnothing, \texttt{H}_{[\texttt{id} \mapsto \texttt{Just a}]} \oplus \texttt{B}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c True}]}\rangle$\\
& \\
$\semlheaprule{ATryTakeMVar id c}{Just x}\rightarrow$\\
\multicolumn{2}{l}{\hfill $\langle\texttt{C}, \varnothing, \texttt{H}_{[\texttt{id} \mapsto \texttt{Nothing}]} \oplus \texttt{B}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c (Just x)}]}\rangle$}\\
& \\
$\semlheaprule{ATryTakeMVar id c}{Nothing}\rightarrow$&
$\langle\texttt{C}, \varnothing, \texttt{H} \oplus \texttt{B}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c Nothing}]}\rangle$\\
& \\
...
\multicolumn{2}{r}{$\langle\texttt{C}, \varnothing, \texttt{H} \oplus \texttt{B}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c ()};~\texttt{id}.\texttt{E} \mapsto \texttt{hs};~\texttt{id}.\texttt{K} \mapsto \texttt{h e}]}\rangle$} \\
...
& \\
$\semlrule{AThrowTo id e c}\rightarrow$&
$\langle\texttt{C}, \varnothing, \texttt{H} \oplus \texttt{B}, \texttt{T}_{[\texttt{tid} \mapsto \semthread{c ()};~\texttt{id} \mapsto \varnothing]}\rangle$ \\
\multicolumn{2}{l}{if $\texttt{interruptible T}_{[\texttt{id}]}$}\\
\multicolumn{2}{l}{\hphantom{if }$\texttt{raise T}_{[\texttt{id}]}~\texttt{e} = \texttt{[]}$}\\
& \\
$\semlrule{ACatching i inner c}\rightarrow$\\
...
& \\
$\semlrule{APopCatching c}\rightarrow$&
...
& \\
$\semlrule{AMasking m act c}\rightarrow$\\
...
Here we only give the semantics of the \verb|atomically| function, a
full operational semantics of STM is given in \cite{harris2005}.  The
STM semantics are small-step reductions of the form
...
this new problem.  There is always a gap between the real
system---GHC's runtime---and the model---\dejafu{}'s semantics.
\dejafu{} is just one component of the Haskell programmer's toolbox,
it is not the be-all and end-all for concurrency testing.  However,
for the class of bugs which \dejafu{} can find, it is much more
effective than simply running the program in \verb|IO| many times.
