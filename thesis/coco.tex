...
programs are observationally equivalent~\sref{dejafu-whatis-bugs},
...
\paragraph{Nondeterminism and Interference}
...
threads, for example see \cite{dodds2015}. \Cref{lst:lockstack} shows
...
\Cref{lst:lockstack2} shows the properties that CoCo discovers with
...
with contention.  \Cref{lst:crefstack} shows our implementation, which
...
problem.  In the \verb|MVar| example in \Cref{sec:coco-example}, we
...
\item The properties we discover are equivalences and refinements
  ...
  concurrent interference~\sref{coco-concerns}.
...
\item We use an approach similar to QuickSpec \parencite{smallbone2017} and
  ...
  \dejafu{} to discover these sets~\sref{coco-hiw}.
...
\item CoCo is most suited to discovering properties about concurrent
  ...
  examples: \verb|MVar| operations~\sref{coco-example}, concurrent
  stacks~\sref{coco-cases-stack}, and
  semaphores~\sref{coco-cases-semaphore}.  The semaphore example
...
\item We provide an integration between \dejafu{} and CoCo, so users
  ...
  properties continue to hold in the future~\sref{coco-dejafu}.
\end{itemize}
...
% This table takes up a full page, so put it at the end of the
% chapter.
